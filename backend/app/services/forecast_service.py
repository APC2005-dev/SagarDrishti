...
from app.models.ml import Forecast, ForecastRun, ForecastSet
...
from ml.inference.forecast_generator import generate_forecasts
    ...
    def run(self, trigger: str = "scheduled", ingestion_run_id: int | None = None, iceberg_ids: list[str] | None = None) -> ForecastOutcome:
        ...
        existing = set(
            self.session.execute(
                select(ForecastSet.iceberg_id, ForecastSet.anchor_observation_id).where(
                    ...
                    ForecastSet.iceberg_id.in_([s.iceberg_id for s in result.sequences] or [""]),
                )
            ).tuples()
        )
        ...
        for fc in generated:
            ...
            fs = ForecastSet(
                ...
                input_features=seq.features.astype(float).round(6).tolist(),
        ...
        run.forecast_sets_created = len(generated)
        status = "success" if not skipped else "partial"
        if not generated and not run.already_forecast:
            status = "skipped" if skipped else "success"
        return self._finish(run, status, skipped=dict(skipped))

    def _finish(self, run: ForecastRun, status: str, skipped: dict[str, int] | None = None, error: str | None = None) -> ForecastOutcome:
        run.status = status
        ...
        log.info("forecast_generation_completed", run_id=run.id, model_version=run.model_version, status=status,
                 created=run.forecast_sets_created, already=run.already_forecast, skipped=skipped, error=error)
