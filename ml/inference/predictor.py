"""Scaled GRU inference (notebook §6–§7), for any registered feature schema."""
...
def scale_features(bundle: ModelBundle, X_raw: NDArray[np.float32]) -> NDArray[np.float32]:
    shape = X_raw.shape
    return bundle.feature_scaler.transform(X_raw.reshape(-1, shape[-1])).reshape(shape).astype(np.float32)
...
def predict_displacements_km(
    ...
    if X_raw.ndim != 3 or X_raw.shape[1:] != (SEQUENCE_LENGTH, n_features):
        raise ValueError(f"expected (n, {SEQUENCE_LENGTH}, {n_features}) for {bundle.version}, got {X_raw.shape}")
    if len(X_raw) == 0:
        return np.zeros((0, FORECAST_DAYS, 2))
    if not np.isfinite(X_raw).all():
        raise ValueError("input contains missing (NaN) values; environmental gaps must be resolved before inference")
...
def stack_features(features: Sequence[NDArray[np.float32]], n_features: int = N_FEATURES) -> NDArray[np.float32]:
    if not features:
