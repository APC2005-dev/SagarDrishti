...
export default function ForecastPage() {
  ...
  return (
    <div className="split narrow" style={{ gridTemplateColumns: 'minmax(380px, 420px) 1fr' }}>
      <section className="split-left scroll pad" aria-label="Forecast summary">
        <div className="page-head">
          <div>
            <span className="label">Trajectory forecast</span>
            <AnimatePresence mode="wait" initial={false}>
              <motion.h1 key={horizon} initial={{ opacity: 0, y: 4 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -4 }} transition={{ duration: 0.18 }}>
        ...
        <SectionTitle right={<span className="mono dim">{rows.length} sets</span>}>Forecasts at D+{horizon}</SectionTitle>
        <QueryState
          query={forecasts}
          empty={(d) => d.length === 0}
        ...
        >
          {() => (
            <table className="data clickable">
              ...
              <tbody>
                {rows.map(({ f, h }) => (
                  <tr key={f.forecastSetId} className={f.icebergId === selected ? 'selected' : undefined} onClick={() => select(f.icebergId === selected ? null : f.icebergId)}>
      ...
      <section className="split-right" aria-label="Forecast map">
