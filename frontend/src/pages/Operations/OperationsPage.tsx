...
export default function OperationsPage() {
  ...
  return (
    <div className="page-scroll">
      ...
      <SectionTitle right={ingestion.data && <FeedStateChip state={ingestion.data.feedState} />}>USNIC ingestion</SectionTitle>
      <QueryState query={ingestion}>
        {(s) => (
          <>
            <div className="cards" style={{ gridTemplateColumns: 'repeat(auto-fill, minmax(170px, 1fr))' }}>
              <Metric label="Last fetch" value={relTime(s.latestRun?.fetchedAt)} sub={s.latestRun ? <RunStatusChip status={s.latestRun.status} /> : 'never'} />
              ...
              <Metric label="Failed rows" value={fmtNum(s.latestRun?.failedRows)} tone={s.latestRun?.failedRows ? 'bad' : 'default'} />
            </div>
            {s.stateReasons.length > 0 && <div className="note warn" style={{ marginTop: 10 }}>{s.stateReasons.join(' · ')}</div>}
                  ...
                  <tbody>
                    {s.recentRuns.map((r) => (
                      <tr key={r.id}>
                        ...
                        <td className="mono dim" title={r.checksumSha256 ?? ''}>{shortHash(r.checksumSha256)}</td>
                        <td className="mono" style={{ color: 'var(--bad)', maxWidth: 280, overflow: 'hidden', textOverflow: 'ellipsis' }} title={r.errorMessage ?? ''}>{r.errorMessage ?? ''}</td>
                      </tr>
                    ))}
                  </tbody>
                </table>
                ...
                {runs.slice(0, 10).map((r) => (
                  <tr key={r.id}>
                    ...
                    <td className="mono dim">
                      {r.errorMessage ?? (Object.entries(r.skipped ?? {}).map(([k, v]) => `${k.replace(/_/g, ' ')}: ${v}`).join(' · ') || '—')}
                    </td>
                  </tr>
                ))}
              </tbody>
      ...
      <QueryState query={models} empty={(m) => m.length === 0} emptyText="No model versions registered. Place the base artifact in models/base and run make bootstrap.">
        {(versions) => (
          <div className="cards">
            <div className="panel card">
              <h3>Lineage</h3>
              <span className="dim mono" style={{ fontSize: 11 }}>base → v1 → … → vN (rejected branches kept)</span>
              ...
              .map((v) => (
                <div key={v.version} className="panel card">
                  ...
                  <div className="kv-grid" style={{ marginTop: 12 }}>
                    ...
                    <KV k="D+1 / D+3 / D+7" v={`${fmtKm(v.day1Error, 2)} / ${fmtKm(v.day3Error, 2)} / ${fmtKm(v.day7Error, 2)}`} />
      ...
      <SectionTitle right={retraining.data?.latestRun && <RunStatusChip status={retraining.data.latestRun.status} />}>Retraining</SectionTitle>
      <QueryState query={retraining}>
        {(r) => (
          <div className="cards">
            <div className="panel card">
              <h3>Eligibility {r.eligibility?.eligible ? <Chip tone="ok">ELIGIBLE</Chip> : <Chip tone="neutral">WAITING</Chip>}</h3>
              <span className="dim mono" style={{ fontSize: 11 }}>since {fmtDateTime(r.eligibility?.since)}</span>
              <div style={{ display: 'flex', flexDirection: 'column', gap: 6, marginTop: 12 }}>
                {Object.entries(r.eligibility?.checks ?? {}).map(([k, c]) => (
                  <div key={k} className="mono" style={{ display: 'flex', justifyContent: 'space-between', fontSize: 12 }}>
            ...
            <div className="panel card" style={{ gridColumn: '1 / -1' }}>
              ...
              {r.recentRuns.length === 0 ? (
                <div className="qs-empty" style={{ marginTop: 8 }}>No retraining runs yet.</div>
              ) : (
                  ...
                  <tbody>
                    {r.recentRuns.map((run) => (
                      <tr key={run.id}>
                        ...
                        <td className="mono dim" style={{ whiteSpace: 'normal' }}>{run.decision?.reasons.join('; ') ?? run.failureReason ?? '—'}</td>
                      </tr>
                    ))}
                  </tbody>
                </table>
